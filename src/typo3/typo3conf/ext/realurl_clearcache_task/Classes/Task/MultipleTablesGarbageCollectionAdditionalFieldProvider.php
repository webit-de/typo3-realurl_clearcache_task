 ...
 */
class MultipleTablesGarbageCollectionAdditionalFieldProvider implements \TYPO3\CMS\Scheduler\AdditionalFieldProviderInterface {

	/**
	 * Dayrange from current to value
	 * In this Range no data deleted from table tstamp
	 *
	 ...
	 *
	 * @return array Array containing all the informationof additional fields
	 */
	public function getAdditionalFields(array &$taskInfo, $task, \TYPO3\CMS\Scheduler\Controller\SchedulerModuleController $parentObject) {
		...
		$additionalFields['task_tableGarbageCollection_truncateTables'] = $this->getTruncateTablesAdditionalField($taskInfo, $task, $parentObject);
	 ...
	 * @param \TYPO3\CMS\Scheduler\Controller\SchedulerModuleController $parentObject Reference to the calling object (Scheduler's BE module)
	 *
	 * @return array Array containing all the information of additional fields
	 */
	 ...
	 * @param \TYPO3\CMS\Scheduler\Controller\SchedulerModuleController $parentObject Reference to the calling object (Scheduler's BE module)
	 *
	 * @return array Array Contain all the information of additional fields
	 */
	protected function getTableAdditionalField(array &$taskInfo, $task, \TYPO3\CMS\Scheduler\Controller\SchedulerModuleController $parentObject) {
		...
		if ($parentObject->CMD === 'edit' && !array_key_exists($task->table, $tableConfiguration) && !empty($task->table)) {
			$options[] = '<option value="" selected="selected"></option>';
		}
		foreach ($tableConfiguration as $tableName => $_) {
			if ($parentObject->CMD === 'add' && count($options) === 0) {
				...
				if (is_array($task->multipleTables) && in_array($tableName, $task->multipleTables)) {
		...
		$fieldHtml[] = '<select name="' . $fieldName . '" ' . $disabled .
				'onChange="actOnChangeSchedulerTableGarbageCollectionTable(this)"' .
				'id="' . $fieldId . '" class="wide" size="10" multiple="multiple">' . implode(LF, $options) . '</select>';
	...
	/**
	 * Add a checkbox field for truncation of tables.
	 *
	 * @param array $taskInfo Reference to the array containing the info used in the add/edit form
	 * @param object $task When editing, reference to the current task object. Null when adding.
	 * @param \TYPO3\CMS\Scheduler\Controller\SchedulerModuleController $parentObject Reference to the calling object (Scheduler's BE module)
	 *
	 * @return array Array containing all the information ofadditional fields
	 */
	protected function getTruncateTablesAdditionalField(array &$taskInfo, $task, \TYPO3\CMS\Scheduler\Controller\SchedulerModuleController $parentObject) {
		if ($parentObject->CMD === 'edit') {
			$checked = $task->truncateTables === TRUE ? 'checked="checked" ' : '';
		} else {
			$checked = '';
		}
		$fieldName = 'tx_scheduler[scheduler_tableGarbageCollection_truncateTables]';
		$fieldId = 'task_tableGarbageCollection_truncateTables';
		$fieldHtml = array();
		...
		$fieldConfiguration = array(
				'code' => implode(LF, $fieldHtml),
				'label' => 'komplettes Leeren der Tabelle(n)',
				'cshKey' => '_MOD_system_txschedulerM1',
				'cshLabel' => $fieldId
		);
		return $fieldConfiguration;
	...
	protected function getNumberOfDaysAdditionalField(array &$taskInfo, $task, \TYPO3\CMS\Scheduler\Controller\SchedulerModuleController $parentObject) {
		...
		if ($task->truncateTables) {
			$disabled = ' disabled="disabled" ';
			$taskInfo['scheduler_tableGarbageCollection_numberOfDays'] = 0;
			$task->numberOfDays = 0;
		} else {
			$disabled = '';
		}
		...
		$fieldHtml = '<input type="text" name="' . $fieldName . '" id="' . $fieldId . '" ' . $disabled .
				'value="' . (int)$taskInfo['scheduler_tableGarbageCollection_numberOfDays'] . '" size="4" />';
		$fieldConfiguration = array(
			'code' => $fieldHtml,
	 ...
	 * @param \TYPO3\CMS\Scheduler\Controller\SchedulerModuleController $parentObject Reference to the calling object (Scheduler's BE module)
	 *
	 * @return bool True if validation was ok (or not relevant), false otherwise
	 */
	public function validateAdditionalFields(array &$submittedData, \TYPO3\CMS\Scheduler\Controller\SchedulerModuleController $parentObject) {
		...
		$validData &= $this->validateTruncateTablesAdditionalField($submittedData, $parentObject);
	...
	public function validateTableAdditionalField(array &$submittedData, \TYPO3\CMS\Scheduler\Controller\SchedulerModuleController $parentObject) {
		$validData = FALSE;
		$tableConfiguration = $GLOBALS['TYPO3_CONF_VARS']['SC_OPTIONS']['scheduler']['tasks']['TYPO3\\CMS\\Scheduler\\Task\\TableGarbageCollectionTask']['options']['tables'];
		if (is_array($submittedData['scheduler_tableGarbageCollection_table']) && !empty($submittedData['scheduler_tableGarbageCollection_table'])) {
				$validData = TRUE;
		} else {
			$parentObject->addMessage('Keine Tabelle gewählt!', \TYPO3\CMS\Core\Messaging\FlashMessage::ERROR);
		}
		return $validData;
	}

	/**
	 * Checks if truncate tables field is correct
	 *
	 * @param array $submittedData Reference to the array containing the data submitted by the user
	 * @param \TYPO3\CMS\Scheduler\Controller\SchedulerModuleController $parentObject Reference to the calling object (Scheduler's BE module)
	 *
	 * @return bool True if data is valid
	 */
	public function validateTruncateTablesAdditionalField(array &$submittedData, \TYPO3\CMS\Scheduler\Controller\SchedulerModuleController $parentObject) {
		$validData = FALSE;
		if (!isset($submittedData['scheduler_tableGarbageCollection_truncateTables'])) {
			$validData = TRUE;
		} elseif ($submittedData['scheduler_tableGarbageCollection_truncateTables'] === 'on') {
			$validData = TRUE;
		}
		return $validData;
	}
	...
	public function validateNumberOfDaysAdditionalField(array &$submittedData, \TYPO3\CMS\Scheduler\Controller\SchedulerModuleController $parentObject) {
		...
		if (!isset($submittedData['scheduler_tableGarbageCollection_numberOfDays'])) {
			...
			// Issue error message
			$parentObject->addMessage($GLOBALS['LANG']->sL('LLL:EXT:scheduler/mod1/locallang.xlf:msg.invalidNumberOfDays'),
	...
	public function saveAdditionalFields(array $submittedData, \TYPO3\CMS\Scheduler\Task\AbstractTask $task) {
		...
		$task->multipleTables = $submittedData['scheduler_tableGarbageCollection_table'];
		$task->truncateTables = $submittedData['scheduler_tableGarbageCollection_truncateTables'] === 'on' ? TRUE : FALSE;
